...
import { ArrowDown, ArrowRight, ArrowUpRight, ChevronLeft, ChevronRight, Pause, Play } from "lucide-react"
import { projects } from "@/lib/data/projects"
import "./portfolio.css"

const stack = [
  { name: "FRONTEND", text: "Fast, responsive interfaces for the web and mobile. Built to feel natural, from the first click to the last screen.", tools: ["TYPESCRIPT", "REACT", "NEXT.JS", "REACT NATIVE", "TAILWIND"] },
  { name: "BACKEND", text: "The systems behind the screen. APIs, databases, and integrations that keep your product running.", tools: ["PYTHON", "FASTAPI", "POSTGRESQL", "SUPABASE", "WEBSOCKETS"] },
  { name: "AI & AUTOMATION", text: "AI that does useful work. Voice agents, retrieval pipelines, and workflows that turn busywork into background work.", tools: ["LANGGRAPH", "LANGCHAIN", "RAG", "N8N", "TENSORFLOW"] },
]
const experience = [
  ["2025 — PRESENT", "FLOWCRAFT", "Founder · AI tooling & automation consultancy"],
  ["JAN — APR 2026", "LEAN AUTOMATION", "Junior Software Engineer · Full-stack & n8n automation"],
  ["AUG — OCT 2025", "DEVELOPERS HUB", "AI Engineer Intern · NLP & LLM integrations"],
  ["2022 — 2026", "COMSATS UNIVERSITY", "Computer Science · Abbottabad campus"],
]
const process = [
  ...
  return (
    <div className="portfolio" id="top">
      ...
      <header className="portfolio-header wrap">
        ...
        <nav aria-label="Main navigation"><a href="#stack">STACK</a><a href="#impact">IMPACT</a><a href="#work">WORK</a><a href="#process">PROCESS</a></nav>
      ...
      <main id="main">
        ...
        <section className="impact-section" id="impact"><div className="wrap section-space">
          ...
          <div className="impact-stats"><div><strong>~70<span>%</span></strong><p>LESS MANUAL PROCESSING</p><span>Client pipelines at Lean Automation</span></div><div><strong>15<span>+</span></strong><p>INTEGRATIONS CONNECTED</p><span>Across client automation pipelines</span></div><div><strong>91<span>%</span></strong><p>DISEASE DETECTION ACCURACY</p><span>FarmGuardian · on-device & offline</span></div></div>
        </div></section>
        <section className="work-section wrap section-space" id="work">
          ...
          <div className="project-rail" ref={rail} onScroll={() => { const el = rail.current; if (el) { const width = (el.children[0] as HTMLElement)?.offsetWidth + 24; setActive(Math.min(projects.length - 1, Math.round(el.scrollLeft / width))) } }}>
            {projects.map((project, i) => <Link href={`/work/${project.slug}`} className={`work-card work-${i}`} key={project.slug}><div className="work-image"><Image src={project.coverImage} alt={`${project.title} project preview`} fill sizes="(max-width: 640px) 85vw, 55vw" /><span className="project-open" aria-hidden="true"><ArrowUpRight /></span></div><div className="work-copy"><span className="micro">№0{i + 1} — {project.category}</span><h3>{project.title}</h3><p>{project.tagline}</p><span className="micro project-tech">{project.techStack.slice(0, 4).join(" · ")}</span></div></Link>)}
          </div>
          <div className="rail-controls"><span className="micro">SCROLL SIDEWAYS OR PICK A PROJECT →</span><div className="project-dots">{projects.map((p, i) => <button key={p.slug} aria-label={`Show project ${i + 1}: ${p.title}`} aria-pressed={active === i} onClick={() => showProject(i)} />)}</div><div className="rail-arrows"><button aria-label="Previous project" disabled={active === 0} onClick={() => showProject(active - 1)}><ChevronLeft /></button><button aria-label="Next project" disabled={active === projects.length - 1} onClick={() => showProject(active + 1)}><ChevronRight /></button></div></div>
        </section>
        <section className="experience-section wrap section-space" id="experience"><SectionTitle number="04" title="THE EXPERIENCE BEHIND IT" note="WORK & EDUCATION" /><div className="experience-list">{experience.map(([date, company, role]) => <article key={company}><span className="micro">{date}</span><h3>{company}</h3><p>{role}</p><ArrowUpRight aria-hidden="true" /></article>)}</div></section>
        <section className="process-section" id="process"><div className="wrap section-space"><SectionTitle number="05" title="HOW THE WORK GOES" note="4 STEPS. A CLEAR PATH." /><ol className="process-grid">{process.map(([title, text], i) => <li key={title}><span className="step-number">0{i + 1}</span><h3>{title}</h3><p>{text}</p></li>)}</ol></div></section>
      </main>
      <footer className="contact-section" id="contact"><div className="wrap"><div className="contact-top micro"><span>// GOT A PROJECT? LET'S MAKE IT WORK.</span><span className="availability"><i /> AVAILABLE FOR FREELANCE</span></div><a className="write-me" href="mailto:farazshoukat1@gmail.com">LET'S TALK <ArrowUpRight aria-hidden="true" /></a><div className="contact-links micro"><a href="mailto:farazshoukat1@gmail.com">FARAZSHOUKAT1@GMAIL.COM ↗</a><a href="https://github.com/farazshoukat" target="_blank" rel="noopener noreferrer">GITHUB ↗</a><a href="https://www.linkedin.com/in/faraz-shoukat-539161289/" target="_blank" rel="noopener noreferrer">LINKEDIN ↗</a><a href="https://instagram.com/faraz.work" target="_blank" rel="noopener noreferrer">INSTAGRAM ↗</a></div><div className="footer-bottom micro"><span>PAKISTAN · REMOTE WORLDWIDE</span><span>© {new Date().getFullYear()} FARAZ SHOUKAT</span><a href="#top">BACK TO TOP ↑</a></div></div></footer>
    </div>
  )
}
