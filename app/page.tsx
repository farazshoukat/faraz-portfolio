    ...
    <PortfolioMotion><div className="portfolio" id="top">
      ...
      <main id="main">
        ...
        <section className="work-section wrap section-space" id="work">
          ...
          <div className="work-filters" role="group" aria-label="Filter projects">{["All", "Full-Stack", "Machine Learning", "AI & Automation", "Mobile", "Interactive"].map(value => <button key={value} className="filter-button micro" aria-pressed={filter === value} onClick={() => selectFilter(value)}>{value}<span>{projects.filter(p => value === "All" || p.filters.includes(value)).length}</span></button>)}</div>
          ...
          <div className="project-rail" key={filter} ref={rail} onScroll={() => { const el = rail.current; if (el) { const width = (el.children[0] as HTMLElement)?.offsetWidth + 24; setActive(Math.min(filteredProjects.length - 1, Math.round(el.scrollLeft / width))) } }}>
            {filteredProjects.map((project, i) => <article className={`work-card work-${i}`} key={project.slug}><Link href={`/work/${project.slug}`} className="work-main"><div className="work-image"><Image src={project.coverImage} alt={`${project.title} project preview`} fill sizes="(max-width: 640px) 85vw, 55vw" /><span className="project-open" aria-hidden="true"><ArrowUpRight /></span></div><div className="work-copy"><span className="micro">№0{i + 1} — {project.category}</span><h3>{project.title}</h3><p>{project.tagline}</p><span className="micro project-tech">{project.techStack.slice(0, 4).join(" · ")}</span></div></Link><div className="work-sources micro">{project.repoUrl ? <a href={project.repoUrl} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.title} repository`}>VIEW CODE <ArrowUpRight size={15} /></a> : <span>PUBLIC REPO NOT AVAILABLE</span>}{project.demoUrl && <a href={project.demoUrl} target="_blank" rel="noopener noreferrer" aria-label={`Open ${project.title} live app`}>LIVE APP <ArrowUpRight size={15} /></a>}</div></article>)}
          </div>
